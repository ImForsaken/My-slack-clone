...
export class DirectMessageDbService {
  ...
  private directMessagesCollRef: CollectionReference = collection(
    this.firestore,
    'directMessages'
  );
  ...
  getAllDirectMessages$() {
    return collectionData(this.directMessagesCollRef, {
      idField: 'id',
    }) as Observable<TDirectMessages[]>;
  }
  ...
  getDirectMessage$(dmId: string): Observable<DocumentData> {
    const directMessagesDocRef: DocumentReference<DocumentData> = doc(
      this.directMessagesCollRef,
      dmId
    );
